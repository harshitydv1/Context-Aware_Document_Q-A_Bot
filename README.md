...
Short explanation of chunking and retrieval approach


Chunking: The document is split into small overlapping chunks (400 chars with 50 overlap) so each piece retains enough context for retrieval. This prevents long documents from exceeding model limits and improves relevance.

Retrieval: Each chunk is embedded into a vector and stored in FAISS. When a user asks a question, the query is embedded, the top‑3 most similar chunks are retrieved, and only those chunks are sent to the LLM to generate the answer.
